...
public class FHIRCreatefromDBfromBP {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    context.addRoutes(new RouteBuilder(){
			...
			public void configure(){
				...
				.setBody(constant("SELECT * FROM " + privateData.bpTable + " WHERE Fid is NULL"))
		        .to("jdbc:myDataSource")
		        .marshal().json(true)
		        .to("file:src/test?fileName=bpdb")
				.process(new Processor() {

					public void process(Exchange exchange) throws Exception {
						
						String body = exchange.getIn().getBody(String.class);				        
				        exchange.getIn().setBody(cp.getFHIRBundles(body, IDlist, "BP"));
						
					}
					
				})
//				.marshal().fhirJson(true) 
//				.to("file:src/test?fileName=bpfhir")
		        ...
		        .process(new Processor() {
					public void process(Exchange exchange) throws Exception {
						...
						String sql = func.getUpdateSql(privateData.bpTable, "Id", result, IDlist);
