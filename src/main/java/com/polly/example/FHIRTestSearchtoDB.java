...
public class FHIRTestSearchtoDB {
			...
			public void configure(){
				...
				.process(new Processor() {
					...
					public void process(Exchange exchange) throws Exception {
						...
						String uid = getID(p.getIdentifier(), code.idenCode) == "" ? "Unknow" : getID(p.getIdentifier(), code.idenCode);  //身分證
						String pid = getID(p.getIdentifier(), code.recordCode)  == "" ? "Unknow" : getID(p.getIdentifier(), code.recordCode);  //病歷號
	...
	private static String getMarrage(CodeableConcept code) {
		if(code.getCoding().get(0).getCode().equals("U")) {
			return "未婚";
		}else if(code.getCoding().get(0).getCode().equals("M")){
			return "已婚";
		}else if(code.getCoding().get(0).getCode().equals("D")){
			return "離婚";
		}else {
			return "Unknown";
		}
	...
	private static String getContactRelation(CodeableConcept code) {
		if(code.getCoding().get(0).getCode().equals("FTH")) {
			return "父";
		}else if(code.getCoding().get(0).getCode().equals("MTH")){
			return "母";
		}else if(code.getCoding().get(0).getCode().equals("SPO")){
			return "配偶";
		}else {
			return "Unknown";
		}
